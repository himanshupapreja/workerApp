...
namespace Worker_7ERFAcraft.ViewModels
{
    public class WorkerWorkDetailViewModel : INotifyPropertyChanged
    {
        ...
        private bool _isDriverList = false;
        public bool IsDriverList
        {
            get
            {
                return _isDriverList;

            }
            set
            {
                _isDriverList = value;
                PropertyChanged(this, new PropertyChangedEventArgs("IsDriverList"));
            }
        }
        ...
        public bool PhotoVisible
        {
            get
            {
                return _photoVisible;

            }
            set
            {
                _photoVisible = value;
                PropertyChanged(this, new PropertyChangedEventArgs("PhotoVisible"));
            }
        }
                ...
                try
                {
                    ...
                    if (result != null)
                    {
                        ...
                        if (result.status)
                        {
                            ...
                            PhotoVisible = result.workDetailData.OrderMedias.Count > 0 ? true : false;
                ...
                return new Command((e) =>
                 {
                    ...
                    {
                        _navigation.PushAsync(new OrderDriversListPopup(driversList));
                    }
                });
            }
        }
                ...
                try
                {
                    ...
                    if (result != null)
                    {
                        ...
                        if (result.status)
                        {
                            if (status == UpdateOrderStatus.Complete)
                            {
                                await App.Current.MainPage.Navigation.PushPopupAsync(new WorkerAcceptPopup(Convert.ToInt32(_orderId), UpdateOrderStatus.Complete));
