...
namespace Worker_7ERFAcraft.Pages
{
    public class HomeMasterPage : MasterDetailPage
    {
        ...
        public HomeMasterPage()
        {
            ...
            MessagingCenter.Subscribe<string>(this, "RefreshHomePage", (sender) =>
            {
                if (LoginUserDetails.userType == (int)UserType.Worker)
                {
                    Master = MasterPage;
                    {
                        Detail = new NavigationPage(new WorkerNewHomePage());
                    }
                }
                else if (LoginUserDetails.userType == (int)UserType.Driver)
                {
                    Master = MasterPage;
                    {
                        Detail = new NavigationPage(new DriverNewHomePage());
                    }
                }
            });
